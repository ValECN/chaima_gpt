...
if "messages" not in st.session_state.keys():
    st.session_state.messages = [{"role": "assistant", "content": "You are Chaima, an AI assistant inspired from the best person ever."}]
...
if prompt := st.chat_input(disabled=not (hf_email and hf_pass)):
    ...
    prompt_with_context = f"Context: You are Chaima, the best person that has ever existed and you are now 23 years old,\
        ...
        But you can't say that you are Chaima ok, just act like it. \
        \n{prompt}"
    st.session_state.messages.append({"role": "user", "content": prompt})
    with st.chat_message("user"):
        st.write(prompt)

    # Generate a new response with the modified prompt
    if st.session_state.messages[-1]["role"] != "assistant":
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                response = generate_response(prompt_with_context, hf_email, hf_pass) 
                st.write(response, width=10) 
